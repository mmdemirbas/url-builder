...
class UrlBuilder {
    ...
    fun addPath    (path              : String, vararg matrixParams: KeyValue) = apply { this.path     += PathSegment(path, matrixParams.asList()) }
    fun addPaths   (vararg paths      : String                               ) = apply { this.path     += paths.map { PathSegment(it) }}
    fun resetPath  (                                                         ) = apply { this.path.clear()                                         }
    fun resetQuery (                                                         ) = apply { this.query    = null                                      }
    fun setQuery   (vararg queryParams: KeyValue                             ) = apply { this.query    = Query.Html4(queryParams.asList())               }
